(() => {
    ...
    window.AzureDevOpsProxy = {
        ...
        getQueryNumberFields: (queryId) => {
            var deferred = $.Deferred();
            var webContext = VSS.getWebContext();

            witRestClient.getQuery(webContext.project.id, queryId, 'all', 1).then(function (query) {
                var dateFields = query.columns.filter(column => {
                    return projectFields.filter(field => field.referenceName == column.referenceName && field.type == 7).length > 0;
                });
            ...
            Promise.all(deferreds).then(result => {
                ...
                var result = {
                    ...
                    columns: board.columns
                        .filter(column => column.columnType != 2)
                        .map(column => {
                            return {
                                id: column.id,
                                name: column.name
                ...
                var query = 'SELECT [System.Id], [System.Title], [System.BoardColumn], [System.State], [System.WorkItemType] ' + 
                            'FROM WorkItems ' + 
